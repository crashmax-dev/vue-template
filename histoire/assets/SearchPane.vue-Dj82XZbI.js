...
function useSelection(list) {
	...
	function selectNext() {
		selectedIndex.value++;
		if (selectedIndex.value > list.value.length - 1) selectedIndex.value = 0;
	}
...
//#endregion
//#region ../../../../../../../__resolved__virtual:$histoire-commands
var registeredCommands = [{
	id: "histoire:plugin-vue:generate-story",
	label: "Generate Vue 3 story from component",
	icon: "https://vuejs.org/logo.svg",
	searchText: "generate create",
	...generate_story_client_default
}];
//#endregion
//#region \0virtual:$histoire-search-title-data
var searchData$1 = {
	"index": [
		{
			"id": 0,
	...
	"idMap": {
		...
		"1": {
			"id": "src-button-story-button-story-vue:src-button-story-button-story-vue-0",
			"kind": "variant"
		},
		"2": {
			"id": "src-button-story-button-story-vue:src-button-story-button-story-vue-1",
			"kind": "variant"
		}
	}
};
var handlers = [];
function onUpdate$1(cb) {
	handlers.push(cb);
}
//#endregion
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("a", {
				class: normalizeClass(["istoire-base-list-ite htw-flex htw-items-center htw-gap-2 htw-text-gray-900 dark:htw-text-gray-100", [_ctx.$attrs.class, __props.isActive ? "active htw-bg-primary-500 hover:htw-bg-primary-600 htw-text-white dark:htw-text-black" : "hover:htw-bg-primary-100 dark:hover:htw-bg-primary-900"]]),
				onClick: _cache[0] || (_cache[0] = ($event) => handleNavigate()),
				onKeyup: [_cache[1] || (_cache[1] = withKeys(($event) => handleNavigate(), ["enter"])), _cache[2] || (_cache[2] = withKeys(($event) => handleNavigate(), ["space"]))]
...
var _sfc_main$2 = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock(Fragment, null, [createVNode(BaseIcon, {
				icon: __props.result.icon ?? defaultIcons[__props.result.kind],
				class: normalizeClass(["htw-w-4 htw-h-4", [!__props.selected ? [__props.result.iconColor ? "bind-icon-color" : {
					"htw-text-primary-500": __props.result.kind === "story",
					"htw-text-gray-500": __props.result.kind === "variant"
				}] : [], { "colorize-black": __props.selected }]])
			}, null, 8, ["icon", "class"]), createBaseVNode("div", _hoisted_1$2, [createBaseVNode("div", _hoisted_2, [createTextVNode(toDisplayString(__props.result.title) + " ", 1), createBaseVNode("span", _hoisted_3, toDisplayString(kindLabels[__props.result.kind]), 1)]), __props.result.path?.length ? (openBlock(), createElementBlock("div", _hoisted_4, [(openBlock(true), createElementBlock(Fragment, null, renderList(__props.result.path, (p, index) => {
				...
				})) : createCommentVNode("", true), createBaseVNode("span", null, toDisplayString(p), 1)]);
...
var SearchItem = /* @__PURE__ */ _export_sfc(/* @__PURE__ */ defineComponent({
	...
	emits: { close: () => true },
	setup(__props, { emit: __emit }) {
		useCssVars((_ctx) => ({ "v1f9aa6ca": __props.result.iconColor }));
		const props = __props;
		const emit = __emit;
		const el = ref();
		...
		function action(fromClick = false) {
			if ("route" in props.result && !fromClick) router.push(props.result.route);
			if ("onActivate" in props.result) props.result.onActivate();
			emit("close");
		}
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", {
				...
				_: 1
			}, 8, ["to", "is-active"])) : createCommentVNode("", true), "onActivate" in __props.result ? (openBlock(), createBlock(_sfc_main$4, {
				key: 1,
				"is-active": unref(selected),
				class: "htw-px-6 htw-py-4 htw-gap-4",
				...
				_: 1
			}, 8, ["is-active"])) : createCommentVNode("", true)], 8, _hoisted_1$1);
		};
	}
...
var _sfc_main = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		function createIndex() {
			return new Fuse([], { keys: ["text"] });
		}
		async function loadSearchIndex(data) {
			titleSearchIndex = createIndex();
			for (const document of data.index) titleSearchIndex.add(document);
		...
		async function loadDocSearchIndex() {
			async function load(data) {
				...
				for (const document of data.index) docSearchIndex.add(document);
				docIdMap = data.idMap;
				if (rateLimitedSearch.value) await searchOnDocField(rateLimitedSearch.value);
			}
		...
		const results = computed(() => {
			...
			const seen = {};
			for (const r of titleResults.value) seen[r.id] = true;
			for (const r of docsResults.value) if (!seen[r.id]) list.push(r);
			return list;
		});
		...
		return (_ctx, _cache) => {
			...
			}, null, 544), [[vModelText, searchInputText.value]])]), unref(rateLimitedSearch) && !results.value.length ? (openBlock(), createBlock(BaseEmpty, {
				key: 0,
				class: "no-animation"
			}, {
				default: withCtx(() => [..._cache[6] || (_cache[6] = [createTextVNode(" No results ", -1)])]),
				_: 1
			})) : results.value.length ? (openBlock(), createElementBlock("div", _hoisted_1, [(openBlock(true), createElementBlock(Fragment, null, renderList(results.value, (result, index) => {
